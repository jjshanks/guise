...
**Two ProgID layouts (#29).** Historically each key stored its choice as a plain `ProgId` *value*. Current Windows 11 builds (observed on 24H2) keep that for `UserChoice` but nest `UserChoiceLatest`'s choice one level deeper — as a `ProgId` value inside a `ProgId` *subkey* — leaving no `ProgId` value on `UserChoiceLatest` itself. Reading only the direct value then yields `""` for the very key Windows resolves clicks through, so guise must read the direct value first and fall back to the nested `…\UserChoiceLatest\ProgId\ProgId` (this is `winreg.readUCProgID`).

**Authoritative key.** Windows 11 24H2+ resolves clicks through `UserChoiceLatest` preferentially (see 3.4), so the verdict judges the **authoritative ProgID**: `UserChoiceLatest` when present, otherwise the legacy `UserChoice`. guise is default iff that ProgID's `HKCU\SOFTWARE\Classes\<ProgID>\shell\open\command` resolves to the **current** `guise.exe`. This fixes both failure modes: a stale `UserChoiceLatest` ProgID that dead-ends every click reads as *not* default even beside a healthy-looking `UserChoice = GuiseHTML` (#9), and a stale `UserChoice = ChromeHTML` left behind by Windows no longer vetoes a valid `UserChoiceLatest = GuiseHTML` that Windows is actually launching (#29). A repaired alias (3.4) whose command now launches the current `guise.exe` counts as default, since clicks reach guise. When `UserChoiceLatest` is absent the legacy `UserChoice` governs alone.
...
`info_cache` also carries, per directory, the signed-in account's `user_name` (the account **email**, e.g. `joe@acme.com`) and, for Google Workspace accounts, a `hosted_domain` (e.g. `acme.com`; consumer/Gmail accounts store the sentinel `NO_HOSTED_DOMAIN`, normalized to empty). `chrome.ResolveAccount(email, hostedDomain)` builds the reverse index — **account → current directory** — so a rule can bind to a profile by account instead of by the brittle directory name (#22, §5.2). Email takes precedence over hosted domain; matching is case-insensitive; and a hosted-domain lookup falls back to the part after `@` in the email when Chrome recorded no `hosted_domain`. Profiles are searched in discovery order (`Default`, then `Profile N`), so the first match wins when several share a domain. Crucially, `ResolveAccount` returns *not found* when Local State is unreadable — unlike `ProfileExists`, which trusts a configured directory on read failure — because an account binding has no literal directory to fall back to, so it must fail closed to Chrome default (§10).
...
- `profile_match` (optional, #22) binds the rule to a profile by **Google account** instead of by `profile_directory` — an object with `email` (exact account email) and/or `hosted_domain` (Workspace domain), e.g. `{ "email": "joe@acme.com" }` or `{ "hosted_domain": "acme.com" }`. It is resolved to the current directory at route time via `chrome.ResolveAccount` (§4.2), so it survives Chrome renumbering profiles across machines. When present (a non-empty `email` or `hosted_domain`) it **takes precedence** over `profile_directory`; an account that resolves to no current profile (removed, or Local State unreadable) drops to Chrome default exactly like a vanished directory (§10, fail-closed). `email` is consulted before `hosted_domain`. Omitted from the file when unset, so existing configs are untouched.
...
- A "Match by account" dropdown in the selected-rule detail pane (#22, §5.2): binds the rule to a profile by Google account (email/Workspace domain) instead of the on-disk directory, storing `profile_match` and overriding the "Profile" dropdown when set. It lists every signed-in discovered profile (by email) plus any account a rule already binds to that Chrome no longer reports, so an existing binding round-trips. Index 0 ("use the profile above") defers to the directory binding.
