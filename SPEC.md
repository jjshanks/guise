...
3. **Download + verify.** Fetch the `guise.exe` asset into the directory holding the running exe, streaming it through a SHA-256 hasher, and compare against the digest in the release's `guise.exe.sha256` asset. **A mismatch is a hard failure**: the partial file is deleted and nothing becomes installable. The verified file is named `guise.exe.new` so it can never clobber the running `guise.exe`. Asset URLs come verbatim from the release JSON, so they are only followed (including each redirect hop) to GitHub-operated hosts — `github.com`, `api.github.com`, `*.githubusercontent.com` — and the download is capped at the asset's declared size, under a hard 64 MB ceiling, so a malformed release can't fill the disk.

   **Trust model (decided).** The SHA-256 check guarantees the installed bytes are exactly the bytes the release published: it defends against in-transit corruption, truncation, and content substituted at the asset URL. It does **not** prove publisher identity — the binary and its checksum are uploaded by the same CI job to the same release, so a compromised GitHub account or CI runner could publish a malicious binary with a matching checksum. Defending against that would require signing releases with a developer-held key whose public half is baked into the updater; that is out of scope at this project's scale, and accepted.
...
If step 2 fails, step 1 is rolled back so the registered path always resolves to a working binary. The leftover `guise.exe.old` cannot be deleted by the outgoing process (it is the image still executing); the **next** tray startup removes it (best-effort: it may take one more start if the old process is still exiting). Startup also sweeps any orphaned `guise.exe.new` left by a download whose apply never completed — pending-update state is in-process only, so after a restart a leftover `.new` is never installable and the next check simply re-downloads.
